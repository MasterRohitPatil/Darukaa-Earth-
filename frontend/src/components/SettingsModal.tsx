...
export const SettingsModal: React.FC<Props> = ({ isOpen, onClose, onKeySaved }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl w-full max-w-md flex flex-col overflow-hidden shadow-2xl transition-colors">
        {/* Header */}
        ...
        <form onSubmit={handleSave} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="AIzaSy..."
              className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-slate-100 text-xs rounded-xl px-3 py-2.5 font-mono focus:outline-none focus:border-emerald-500"
              required
            />
            <span className="text-[10px] text-slate-500 mt-1 block">
              Keys are stored locally in <code className="text-slate-600 dark:text-slate-400">backend/.env</code> and never exposed to the public.
            </span>
          </div>

          {status === 'success' && (
            <div className="bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-300 dark:border-emerald-800 text-emerald-800 dark:text-emerald-300 p-2.5 rounded-lg text-xs flex items-center gap-2">
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="px-3 py-2 text-xs text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-white rounded-lg transition"
            >
              Cancel
            </button>
            <button
              ...
              className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-xs font-semibold transition disabled:opacity-50 flex items-center gap-1.5 cursor-pointer shadow-xs"
            >

  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden shadow-2xl">
        ...
        {/* Search Bar */}
        <form onSubmit={handleSearch} className="px-6 py-3 border-b border-slate-800 bg-slate-950/50 flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-slate-900 border border-slate-700 text-slate-200 text-xs rounded-xl pl-9 pr-3 py-2 focus:outline-none focus:border-emerald-500"
            />
          </div>
          <button
            type="submit"
            className="px-4 py-2 bg-emerald-700 hover:bg-emerald-600 text-white text-xs font-semibold rounded-xl transition cursor-pointer"
          >
            Filter
          </button>
        </form>
          ...
          {loading ? (
            ...
            evidenceList.map((doc) => (
              ...
              >
                ...
                <div className="bg-slate-900/60 p-2.5 rounded-lg border border-slate-800/80 text-xs text-slate-300">
                ...
                <div className="text-xs text-slate-400 italic bg-slate-900/30 p-2.5 rounded border border-slate-800/40">
                  <span className="text-[10px] text-emerald-400 uppercase font-bold not-italic block mb-1">

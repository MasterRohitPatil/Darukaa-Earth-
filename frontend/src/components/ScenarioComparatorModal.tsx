          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              >
                ...
                <div className="text-[11px] text-slate-400">SOC & Moisture Buffer</div>
              </button>
              ...
              >
                ...
                <div className="text-[11px] text-slate-400">Pollinators & IPM Refugia</div>
              </button>
              ...
              >
                ...
                <div className="text-[11px] text-slate-400">Microclimate Thermal Buffer</div>
              </button>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-2">
                <div className="text-xs font-bold text-indigo-300">{result.title}</div>
                <div className="text-xs text-slate-300 leading-relaxed">{result.mechanism}</div>
              </div>

              {/* Side-by-side comparison */}
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-slate-950/80 p-3.5 rounded-xl border border-slate-800">
                  <div className="text-xs font-semibold text-slate-400 mb-2 uppercase tracking-wider text-[10px]">
                    Current Baseline Pressures
                  </div>
                  <div className="space-y-1.5">
                    {result.baseline_pressures.length === 0 ? (
                      <div className="text-xs text-slate-500 italic">No baseline pressures detected</div>
                    ) : (
                      result.baseline_pressures.map((p: string, i: number) => (
                        <div key={i} className="text-xs text-amber-300 bg-amber-950/30 px-2.5 py-1.5 rounded border border-amber-900/40">
                          ⚠ {p}
                        </div>
                      ))
                    )}
                ...
                <div className="bg-slate-950/80 p-3.5 rounded-xl border border-emerald-900/50">
                  <div className="text-xs font-semibold text-emerald-400 mb-2 uppercase tracking-wider text-[10px]">
                    Projected Scenario Outcomes
                  </div>
                  <div className="space-y-1.5">
                    {result.projected_improvements.map((item: any, i: number) => (
                      <div key={i} className="text-xs text-emerald-300 bg-emerald-950/40 px-2.5 py-1.5 rounded border border-emerald-800/40 flex items-center justify-between">
              ...
              {/* Disclaimer */}
              <div className="bg-slate-950/40 border border-slate-800 p-3 rounded-lg flex items-start gap-2 text-[11px] text-slate-400">
